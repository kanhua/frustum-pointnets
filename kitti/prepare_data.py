...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
                         perturb_box2d=False, augmentX=1, type_whitelist=['Car'], use_lyft=False):
    ...
    if use_lyft == False:
        ...
        kitti_from_lyft_data_path = "/Users/kanhua/Downloads/kitti_format/"
        dataset = KittiFromLyft(kitti_from_lyft_data_path, split="prelim")
        data_idx_list = []
        for file in os.listdir(os.path.join(kitti_from_lyft_data_path,'prelim', "label_2")):
            file_idx=os.path.splitext(file)[0]
            data_idx_list.append(file_idx)
    ...
    for data_idx in data_idx_list:
        ...
        # Rescale the intensity of Lyft data
        if use_lyft:
            pc_rect[:,3]=0.2
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gen_lyft_val', action='store_true', help="Generte KITTI format dataset converted from Lyft")
    ...
    if args.gen_lyft_val:
        extract_frustum_data( \
            '',
            'training',
            os.path.join(BASE_DIR, output_prefix + 'lyft_val.pickle'),
            viz=False, perturb_box2d=False, augmentX=1,
            type_whitelist=['car'], use_lyft=True)
